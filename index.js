...
function getArtistByIndex(artist,number) {
 console.log("the artist at index " +number +" is "+artist[number].name);
} 
...
function get20s(artist){
  ...
  for( i = 0; i < artist.length; i++)
  {
    ...
    if(artist[i].years<=2000 && artist[i].years>=1900)
    {
      newartist.push(artist[i].name);
    }
  }
  return newartist;
}
...
function addArtist(artist){
    ...
    element.years="2001-2021";
    element.genre="Web Design";
    element.nationality="American";
    element.bio="test bio";
    artist.push(element);
    return artist;
  }
